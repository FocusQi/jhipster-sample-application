...
export class BiddingQuotationHeaderUpdateComponent implements OnInit {
  isSaving = false;
  pricecompares: IBiddingPriceCompare[] = [];
  issuanceDateDp: any;
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ biddingQuotationHeader }) => {
      ...
      this.biddingPriceCompareService
        ...
        .pipe(
          map((res: HttpResponse<IBiddingPriceCompare[]>) => {
            return res.body || [];
          })
        )
        .subscribe((resBody: IBiddingPriceCompare[]) => {
          if (!biddingQuotationHeader.priceCompare || !biddingQuotationHeader.priceCompare.id) {
            ...
            this.biddingPriceCompareService
              ...
              .pipe(
                map((subRes: HttpResponse<IBiddingPriceCompare>) => {
                  return subRes.body ? [subRes.body].concat(resBody) : resBody;
                })
              )
              .subscribe((concatRes: IBiddingPriceCompare[]) => (this.pricecompares = concatRes));
          }
        });
    });
  }
